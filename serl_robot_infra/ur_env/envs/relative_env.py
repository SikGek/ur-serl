...
class RelativeFrame(gym.Wrapper):
    ...
    def step(self, action: np.ndarray):
        ...
        transformed_action = self.transform_action(action)   # relative -> base

        obs, reward, done, truncated, info = self.env.step(transformed_action)

        if "intervene_action" in info:
            info["intervene_action_base"] = np.asarray(info["intervene_action"]).copy()
            info["intervene_action"] = self.transform_action_inv(info["intervene_action"])  # base -> relative
